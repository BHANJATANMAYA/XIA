...
:do_install_python
"%XIA_PY_INSTALLER%" /quiet InstallAllUsers=0 PrependPath=1 Include_pip=1 Include_test=0 Include_doc=0
if !errorlevel! neq 0 (
    echo.
...
if not defined SYS_PYTHON (
    ...
    echo   Close this window and re-run launch.bat.
    echo.
    pause >nul
    exit /b 1
)
...
"%XIA_PYTHON%" -m pip install -r "%REQ_FILE%" --quiet --disable-pip-version-check
if !errorlevel! neq 0 (
    echo.
...
:: Kill existing instance so it restarts with our env vars
...
:do_install_ollama
"%XIA_OLLAMA_INSTALLER%" /S >nul 2>&1
...
if !errorlevel! neq 0 (
    ...
    echo   Restart this terminal or your machine, then re-run launch.bat.
    echo.
    pause >nul
    exit /b 1
)
